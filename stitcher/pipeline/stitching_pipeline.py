...
from stitcher.config import DEFAULT_PIPELINE_CONFIG
...
class StitchingPipeline:
    ...
    def _stitch_pair(self, moving_img, base_img, pair_index=1, total_pairs=1, global_step_offset=0):
        ...
        edge1, _ = canny_edge_detect(
            moving_img,
            low_thresh=self.config["CANNY_LOW_THRESH"],
            high_thresh=self.config["CANNY_HIGH_THRESH"],
            sigma=self.config["CANNY_SIGMA"]
        )
        edge2, _ = canny_edge_detect(
            base_img,
            low_thresh=self.config["CANNY_LOW_THRESH"],
            high_thresh=self.config["CANNY_HIGH_THRESH"],
            sigma=self.config["CANNY_SIGMA"]
        ...
        h_matrix = registerTexture(
            moving_img, edge1, base_img, edge2,
            sift_edge_thresh=self.config["SIFT_EDGE_THRESH"],
            sift_ratio_thresh=self.config["SIFT_RATIO_THRESH"],
            hist_shift_bin_ratio=self.config["HIST_SHIFT_BIN_RATIO"],
            ransac_reproj_thresh=self.config["RANSAC_REPROJ_THRESH"]
        )
        ...
        _, _, overlap = compute_overlap_masks(img1_w, img2_w, valid1, valid2)
        ...
        label_map = graph_cut_seam(
            ...
            valid1, valid2,
            overlap,
            saliency_weight=self.config["GC_SALIENCY_WEIGHT"],
            object_weight=self.config["GC_OBJECT_WEIGHT"],
            edge_penalty=self.config["GC_EDGE_PENALTY"],
            mode=self.config.get("GC_MODE", "normal")
        )
