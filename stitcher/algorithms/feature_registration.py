...
from stitcher.config import SIFT_EDGE_THRESH, SIFT_RATIO_THRESH, HIST_SHIFT_BIN_RATIO, RANSAC_REPROJ_THRESH

logger = get_logger(__name__)


def registerTexture(
    img1, edge1, img2, edge2,
    sift_edge_thresh=None,
    sift_ratio_thresh=None,
    hist_shift_bin_ratio=None,
    ransac_reproj_thresh=None
):
    sift_edge_thresh = SIFT_EDGE_THRESH if sift_edge_thresh is None else sift_edge_thresh
    sift_ratio_thresh = SIFT_RATIO_THRESH if sift_ratio_thresh is None else sift_ratio_thresh
    hist_shift_bin_ratio = HIST_SHIFT_BIN_RATIO if hist_shift_bin_ratio is None else hist_shift_bin_ratio
    ransac_reproj_thresh = RANSAC_REPROJ_THRESH if ransac_reproj_thresh is None else ransac_reproj_thresh

    gray1 = to_gray_uint8(img1)
    gray2 = to_gray_uint8(img2)

    mask1 = (edge1 > 0).astype(np.uint8) * 255
    mask2 = (edge2 > 0).astype(np.uint8) * 255
